...
build/sound-drop.exe: build/
	g++ -g -Wall -Wextra  -I./include/ -I$(SOLOUD_HEADERS) -I$(IMGUI_DIR) -I$(IMGUI_DIR)/backends -L$(SOLOUD_A)  src/glad.c src/sound-drop.cpp src/Line.cpp src/Ball.cpp src/util.cpp src/Spawner.cpp src/Interactable.cpp src/SaveState.cpp $(IMGUI_SOURCES) -o build/sound-drop.exe -lglfw -lGL -lsoloud_static -lasound
build/ :
	mkdir build
clean : 
	rm -rf build
